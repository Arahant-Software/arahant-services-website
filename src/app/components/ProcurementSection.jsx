...
import { Cpu, Gauge, Settings, Camera, ShieldCheck, Truck, Clock3, Package } from "lucide-react";
import { FadeUp, FadeLeft, StaggerContainer, StaggerChild } from "./ui/Motion";

const productCategories = [
  { icon: Cpu, title: "PLC Systems", description: "PLC CPUs, I/O modules and communication hardware." },
  { icon: Gauge, title: "Drives & Motion", description: "Industrial drives, VSDs, servo motors and controllers." },
  { icon: Settings, title: "Pneumatics", description: "Actuators, valve assemblies and pneumatic systems." },
  { icon: Camera, title: "Sensors & Vision", description: "Industrial sensors, machine vision and detection systems." },
];
...
const rightCards = [
  { icon: Truck, title: "Rapid Dispatch", text: "Fast sourcing and delivery throughout New Zealand and the Pacific region." },
  { icon: ShieldCheck, title: "Genuine OEM Parts", text: "Guaranteed authentic and fully traceable components from trusted manufacturers." },
  { icon: Clock3, title: "Critical Spares Support", text: "Supporting maintenance shutdowns and emergency breakdown requirements." },
];
    ...
    <section className="relative overflow-hidden bg-gradient-to-br from-slate-50/80 via-white to-orange-50/30 py-20 lg:py-32">
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-5 sm:px-8 lg:px-12">
        ...
        <div className="mt-12 lg:mt-20 grid gap-8 lg:grid-cols-[1.3fr_0.7fr]">
          {/* Left Content */}
          <FadeLeft>
            <div className="rounded-3xl border border-white/60 bg-white/70 backdrop-blur-md p-6 sm:p-10 shadow-[0_20px_60px_rgba(249,115,22,0.08)] h-full">
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                ...
                <h3 className="text-xl sm:text-2xl font-bold text-slate-900">Strategic Industrial Procurement</h3>
              </div>
              ...
              <StaggerContainer stagger={0.06} delay={0.1} className="mt-8 grid gap-3 sm:grid-cols-2">
          ...
          {/* Right Cards */}
          <StaggerContainer stagger={0.12} delay={0.05} className="flex flex-col gap-5">
            {rightCards.map((item) => {
              const Icon = item.icon;
              return (
                <StaggerChild key={item.title}>
                  <motion.div
                    whileHover={{ y: -4, borderColor: "#fdba74", boxShadow: "0 20px_60px rgba(249,115,22,0.15)" }}
                    transition={{ duration: 0.25 }}
                    className="rounded-3xl border border-white/60 bg-white/70 backdrop-blur-md p-6 shadow-[0_10px_40px_rgba(0,0,0,0.05)]"
                  >
                    <motion.div
        ...
        <FadeUp delay={0.05}>
          <div className="mt-16 lg:mt-24">
            <h3 className="text-center text-2xl sm:text-3xl font-bold text-slate-900">Procurement Portfolio</h3>
            <StaggerContainer stagger={0.1} delay={0.05} className="mt-8 lg:mt-12 grid gap-5 sm:grid-cols-2 xl:grid-cols-4">
              {productCategories.map((item) => {
                const Icon = item.icon;
                return (
                  <StaggerChild key={item.title}>
                    <motion.div
                      whileHover={{ y: -8, borderColor: "#fdba74", boxShadow: "0 25px 80px rgba(249,115,22,0.15)" }}
                      transition={{ duration: 0.3 }}
                      className="group relative overflow-hidden rounded-3xl border border-white/60 bg-white/70 backdrop-blur-md p-6 sm:p-8 shadow-[0_10px_40px_rgba(0,0,0,0.05)] h-full"
                    >
